...
self.onmessage = async (e) => {
  ...
  if (msg.type === 'init') {
    ...
    try {
      ...
      // vits uses the wrapper's own defaults, which the build patches for
      // threading; kitten needs the explicit config above.
      tts = arch === 'kitten' ? createOfflineTts(Module, kittenConfig()) : createOfflineTts(Module);
